...
export function usePopoverLogic() {
  ...
  return { isOpen, openPopover, closePopover, togglePopover, uniqueId, note, setNote }
}

type PopoverRootProps = ComponentProps<typeof motion.div>

export function PopoverRoot({ children, className, ...props }: PopoverRootProps) {
...
export function PopoverButton({ children, className }: PopoverTriggerProps) {
    const { isOpen, togglePopover, uniqueId } = usePopover()
    ...
    return (
      <TextureButton
        ...
        size="icon"
        className={cn(
          "text-zinc-950 p-2 dark:border-zinc-50/10 dark:bg-zinc-700 dark:text-zinc-50",
          className
        )}
        onClick={togglePopover}
      >
        <motion.span layoutId={`popover-label-${uniqueId}`} className="text-sm ">
           {isOpen?<X size={16} /> : <Expand size={16} />}
          {children}
        </motion.span>
      </TextureButton>
    )
...
export function PopoverWindow({ children, className, ...props }: ComponentProps<typeof motion.div>) {
    const { isOpen, uniqueId } = usePopover()
...
type PopoverContentProps = {
  children: React.ReactNode
  className?: string
} & ComponentProps<typeof CardStyled>

export function PopoverContent({ children, className, childClassName, ...props }: PopoverContentProps) {
    ...
    <AnimatePresence>
      {isOpen && (
        <CardStyled
          ...
          childClassName={cn("w-auto", childClassName)}
          style={{
            left: 0,
          }}
          {...props}
...
export function PopoverBaseButton({
  children,
  onClick,
  className,
}: {
